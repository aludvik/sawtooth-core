...
class IncomingBatchSender_ErrorCode(IntEnum):
    Success = 0
...
class IncomingBatchSender(OwnedPointer):
    ...
    def send(self, item):
        ...
        if res == IncomingBatchSender_ErrorCode.Success:
            return
        elif res == IncomingBatchSender_ErrorCode.NullPointerProvided:
            raise TypeError("Provided null pointer(s)")
        elif res == IncomingBatchSender_ErrorCode.InvalidInput:
            raise ValueError("Input was not valid ")
        elif res == IncomingBatchSender_ErrorCode.Disconnected:
            raise Disconnected()
        else:
...
class BlockPublisher(OwnedPointer):
    ...
    def _to_exception(res):
        if res == BlockPublisher_ErrorCode.Success:
            return
        elif res == BlockPublisher_ErrorCode.NullPointerProvided:
            raise TypeError("Provided null pointer(s)")
        elif res == BlockPublisher_ErrorCode.InvalidInput:
            raise ValueError("Input was not valid ")
