...
//
// The consensus engine should listens for notifications from the validator about the status of
...
//
// Based on the updates the engine receives through the `Receiver<Update>` and the specifics of the
// algorithm being implemented, the engine utilizes the provided `Service` to create new blocks,
// communicate with its peers, request that certain blocks be committed, and fail or ignore blocks
...
// optimization reasons, it is the consensus engines responsibility to drive the progress of the
// validator and ensure liveness.
//
// It is not the engine's responsibility to manage blocks or memory, other than to ensure it
// responds to every new block with a commit, fail, or ignore within a "reasonable amount of time".
// The validator is responsible for guaranteeing the integrity of all blocks sent to the engine
// until the engine responds. After the engine responds, the validator does not guarantee that the
// block and its predecessors continue to be available unless the block was committed.
//
// Finally, as an optimization, the consensus engine can sends prioritized lists of blocks to the
